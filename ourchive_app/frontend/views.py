...
def user_subscriptions_manage(request, username):
    if not request.user.is_authenticated or not request.user.username == username:
        messages.add_message(request, messages.ERROR, _('You do not have permission to view these subscriptions.'), 'subscription-not-authed')
        return redirect('/')
    works = do_get(f'api/users/{username}/subscriptions/works', request, 'Subscription')
    collections = do_get(f'api/users/{username}/subscriptions/collections', request, 'Subscription')
    users = do_get(f'api/users/{username}/subscriptions/users', request, 'Subscription')
    subscriptions = ((works.response_data['results'] if 'results' in works.response_data else [])
                     + (collections.response_data['results'] if 'results' in collections.response_data else [])
...
def subscribe(request, chive_id=None):
    if not request.user.is_authenticated:
    ...
    if not chive_id:
        ...
        object_type = request.GET.get('obj_type', '')
        if int(request.GET.get('subscribed', 0)) > 0:
            response = do_delete(f'api/users/{request.user}/subscriptions/{object_type}s/{request.GET.get('subscribed', 0)}', request, object_name='Subscription')
            process_message(request, response)
            return referrer_redirect(request)
        post_data[f'{object_type}'] = chive_id
        post_data['user'] = request.user.id
        response = do_post(f'api/users/{request.user}/subscriptions/{object_type}s/create', request, data=post_data, object_name='Subscription')
        process_message(request, response)
        return referrer_redirect(request)
