...
def create_work_zip(work, creator_name):
	...
	with ZipFile(location, 'w') as work_file:
		for chapter in work.chapters:
			...
			work_file.write(get_chapter_concat(work.uid, chapter.number, chapter.title, '.html'), get_filename(chapter.number, chapter.title, '.html'))
			if chapter.image_url is not None:
				work_file.write(get_chapter_concat(work.uid, chapter.number, chapter.title, '.'+chapter.image_format),  get_filename(chapter.number, chapter.title, '.'+chapter.image_format))
			if chapter.audio_url is not None:
				work_file.write(get_chapter_concat(work.uid, chapter.number, chapter.title,".mp3"), get_filename(chapter.number, chapter.title, '.mp3'))
	return location 
